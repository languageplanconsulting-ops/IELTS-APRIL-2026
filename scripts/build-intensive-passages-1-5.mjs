#!/usr/bin/env node
/**
 * Generate intensive journey ด่าน 1–5 from older Cambridge books (P1+P2 per test).
 * Sequence: 10-T1, 10-T2, 10-T3, 10-T4, then 9-T1 (see STAGE_SOURCES).
 * Source: Engnovate + server/*.mjs (Short Thai Explanation + answer keys).
 */
...
/** Cambridge 10 T1–T4 then 9 T1 (ด่าน 5 uses 11 T1 until Cam 9 is imported). */
const STAGE_SOURCES = {
  ...
  // TODO: replace with cambridge-9-test1-* when books 1–9 are scraped
  5: ['cambridge-11-test1-passage1', 'cambridge-11-test1-passage2']
}

const c10 = (await import('../server/userProvidedReadingPracticeCambridge10.mjs'))
  .USER_PROVIDED_READING_PRACTICE_CAMBRIDGE_10_EXAMS
const c11 = (await import('../server/userProvidedReadingPracticeCambridge11.mjs'))
...
const pool = [...c10, ...c11].map(repairExamPayload)
...
const needsFillNotes = Object.values(layoutsByStage).some((layouts) =>
  layouts.some((layout) => layout.sections.some((s) => s.key === 'fill' && s.variant === 'notes'))
)

const passagesOut = []
passagesOut.push(`/**
 * Normal Reading Journey ด่าน 1–5 — Cambridge 10 (T1–T4) + placeholder for 9 T1.
 * Source: Engnovate / server practice data (P1 + P2 only per stage).
 */
...
${needsFillNotes ? `const fillNotes = (summaryTitle: string, items: IntensiveQuestionSpec[]) => ({
...
passagesOut.push(`export const INTENSIVE_LAYOUTS_STAGE_1_5: Record<number, [IntensivePassageLayout, IntensivePassageLayout]> = {
`)
...
fs.writeFileSync(path.join(root, 'src/journeyIntensivePassages1to5.ts'), passagesOut.join('\n'))
...
solLines.push(`}\n`)
fs.writeFileSync(path.join(root, 'src/intensiveJourneyQuestionSolutions1to5.ts'), solLines.join('\n'))
...
fillLines.push(`]\n`)
fs.writeFileSync(path.join(root, 'src/intensiveJourneyFillBlankStages1to5.ts'), fillLines.join('\n'))

console.log('Wrote journeyIntensivePassages1to5.ts, intensiveJourneyQuestionSolutions1to5.ts, intensiveJourneyFillBlankStages1to5.ts')
