/**
 * Rebuild Cambridge 14 reading exams from official PDF OCR extracts + existing answer keys.
 *
 * 1. python3 scripts/extract-cambridge-14-from-pdf.py "/path/to/cambridge-14_compress.pdf"
 * 2. node scripts/build-cambridge-14-reading-from-pdf.mjs
 */
...
const passDir = path.join(__dirname, 'cambridge-14-passages')
const quesDir = path.join(__dirname, 'cambridge-14-questions')
const outputPath = path.join(root, 'server/userProvidedReadingPracticeCambridge14.mjs')
const importJsonPath = path.join(root, 'cambridge-reading-imports/cambridge-14-reading-import.json')

const EXAM_MAP = [
  { id: 'cambridge-14-test1-passage1', key: 't1p1', test: 1, passage: 1 },
  { id: 'cambridge-14-test1-passage2', key: 't1p2', test: 1, passage: 2 },
  { id: 'cambridge-14-test1-passage3', key: 't1p3', test: 1, passage: 3 },
  { id: 'cambridge-14-test2-passage1', key: 't2p1', test: 2, passage: 1 },
  { id: 'cambridge-14-test2-passage2', key: 't2p2', test: 2, passage: 2 },
  { id: 'cambridge-14-test2-passage3', key: 't2p3', test: 2, passage: 3 },
  { id: 'cambridge-14-test3-passage1', key: 't3p1', test: 3, passage: 1 },
  { id: 'cambridge-14-test3-passage2', key: 't3p2', test: 3, passage: 2 },
  { id: 'cambridge-14-test3-passage3', key: 't3p3', test: 3, passage: 3 },
  { id: 'cambridge-14-test4-passage1', key: 't4p1', test: 4, passage: 1 },
  { id: 'cambridge-14-test4-passage2', key: 't4p2', test: 4, passage: 2 },
  { id: 'cambridge-14-test4-passage3', key: 't4p3', test: 4, passage: 3 }
]
...
const { USER_PROVIDED_READING_PRACTICE_CAMBRIDGE_14_EXAMS: existingExams } = await import(
...
const exams = EXAM_MAP.map(({ id, key, test, passage }) => {
  ...
  const title = `Cambridge 14 Test ${test} Passage ${passage} - ${passText.split('\n')[0].trim().replace(/^['"]|['"]$/g, '')}`
...
const fileBody = `export const USER_PROVIDED_READING_PRACTICE_CAMBRIDGE_14_EXAMS = ${JSON.stringify(exams, null, 2)}
`
...
console.log(`Wrote ${exams.length} Cambridge 14 reading exams (${qTotal} questions)`)
