...
const trimGrammarPhrase = (value, maxWords = 5) =>
...
const normalizeNoteText = (value) => String(value || '').trim().replace(/\s+/g, ' ')

const snapPhraseToCueText = (cues, phrase) => {
...
const findCueForPhrase = (cues, phrase) => {
  const lowerPhrase = normalizeNoteText(phrase).toLowerCase()
  return (
...
const vocabPriority = (term) => (DISCOURSE_MARKERS.has(term.toLowerCase()) ? 1 : 5)

const isUsefulGrammarPhrase = (phrase) => {
...
const collectQualityNotes = (cues) =>
  cues.flatMap((cue) => (cue.notes || []).filter(isQualityNote))

export const suggestSpeakingSampleSubtitleNotes = (cues, transcript = '') => {
...
export const enrichSpeakingSampleSubtitlesWithNotes = (cues, transcript = '') => {
  ...
  const nextCues = cues.map((cue) => ({ ...cue, notes: [] }))
