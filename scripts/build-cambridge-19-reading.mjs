...
const importJsonPath = path.join(root, 'cambridge-reading-imports/cambridge-19-reading-import.json')
const outputPath = path.join(root, 'server/userProvidedReadingPracticeCambridge19.mjs')
...
const fileBody = `export const USER_PROVIDED_READING_PRACTICE_CAMBRIDGE_19_EXAMS = ${JSON.stringify(exams, null, 2)}
`
...
console.log(`Wrote ${exams.length} Cambridge 19 reading exams (${qTotal} questions) to ${outputPath}`)
